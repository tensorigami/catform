...
mod tests {
    ...
    #[test]
    fn roundtrip_rmsnorm() {
        let source = std::fs::read_to_string("../models/qwen3/model.cat").unwrap();
        let m = parse::parse(&source);
        let formatted = format_cat(&m, 100);
        let m2 = parse::parse(&formatted);
        let formatted2 = format_cat(&m2, 100);
        assert_eq!(formatted, formatted2, "format is not idempotent");
    }
    ...
    #[test]
    fn format_matches_python_qwen3() {
        let source = std::fs::read_to_string("../models/qwen3/model.cat").unwrap();
        let m = parse::parse(&source);
        let formatted = format_cat(&m, 100);
        // The source should already be canonically formatted
        assert_eq!(
            formatted, source,
            "Rust formatter output differs from canonical .cat"
        );
    }

    #[test]
    fn format_matches_python_moe() {
        let source = std::fs::read_to_string("../models/qwen3_moe/model.cat").unwrap();
        let m = parse::parse(&source);
        let formatted = format_cat(&m, 100);
        assert_eq!(
